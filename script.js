...
if (storageAvailable("localStorage")) {
  ...
  console.log(
    "Sorry, LocalStorage doen't work; cant save light-night settings"
  );
}
...
playBtn.addEventListener("click", () => {
  ...
  playBtnPressed = true;
  if (
    xBoard.getAttribute("class") === "board-selected" &&
    yBoard.getAttribute("class") === "board-selected"
  ) {
    gameMode = "2-players";
    whichSide = "both";
  ...
  const playGame = (e) => {
    ...
    if (!win() && !draw() && playBtnPressed) {
      ...
      while (
        ...
        let random01 = 0;
        while (
          board[random10][random01] === "X" ||
          board[random10][random01] === "Y"
        ) {
